...
source("functions/mtw_copernicus_forecast.R")
```
...
First we compute a raster of marine thermal wave events - that is identifying data that exceed the 90th percentile (or fall below the 10th percentile) of sea surface temperature for that day of year over the last 30 years. It is important to note that these show the pixels that are exceed or fall below the 90th (red) or 10th (blue) percentile.  That is not necessarily indicative of the presence of a heat/cold wave, so this is just the first step. 
...
Next we compute the presence and duration of heat/cold waves.  Here is the heat/cold wave map.  The deeper the red or blue coloring the longer the duration of the thermal event. 
